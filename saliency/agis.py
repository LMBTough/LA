...
def agi1(model, data, target, epsilon=0.05, max_iter=20, topk=1):
    assert len(data.shape) == 4, "Input data must be 4D tensor"
    random.seed(3407)
    selected_ids = random.sample(list(range(0, 999)), topk)
    output = model(data)
...
def agi2(model, data, target, epsilon=0.05, max_iter=20, topk=2):
    assert len(data.shape) == 4, "Input data must be 4D tensor"
    random.seed(3407)
    selected_ids = random.sample(list(range(0, 999)), topk)
    output = model(data)
...
def agi8(model, data, target, epsilon=0.05, max_iter=20, topk=8):
    assert len(data.shape) == 4, "Input data must be 4D tensor"
    random.seed(3407)
    selected_ids = random.sample(list(range(0, 999)), topk)
    output = model(data)
...
def agi14(model, data, target, epsilon=0.05, max_iter=20, topk=14):
    assert len(data.shape) == 4, "Input data must be 4D tensor"
    random.seed(3407)
    selected_ids = random.sample(list(range(0, 999)), topk)
    output = model(data)
